    ...
    <div className="w-full max-w-6xl py-[40px] px-[15px] my-0 mx-auto flex transition-all max-768:flex-col">
      <div id="recent-post" className="w-3/4 max-768:w-full">
        {posts?.map((post, index) => (
          <div id="recent-post-item" className="h-40 py-4 px-4 bg-white rounded-xl overflow-hidden flex flex-col justify-center max-600:w-full max-768:flex-col dark:bg-dacard" key={index}>
            ...
            <div className="w-full flex flex-wrap text-xs text-gray-600 divide-x-2 divide-solid divide-gray-200 dark:text-slate-200">
              ...
              <Link href={`/tags/${post.meta?.tags}`} className="h-max px-2 flex items-center text-pretty"><Icon icon="mdi:tag-outline" className="mr-1 divide-x-2 divide-dashed divide-orange-600" />{post.meta?.tags}</Link>
              <Link href={`/categories/${post.meta?.categories}`} className="h-max px-2 flex items-center text-pretty"><Icon icon="material-symbols:category-outline" className="mr-1" />{post.meta?.categories}</Link>
